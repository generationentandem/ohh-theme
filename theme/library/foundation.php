...
if ( ! function_exists( 'foundationpress_active_nav_class' ) ) :
    function foundationpress_active_nav_class( $classes, $item, $args ) {
        if ( is_active( $item, $args->walker->primary_category ) ) {
            $classes[] = 'active';
            if ( $args->walker instanceof Und_Walker_Nav_Menu && $item->menu_item_parent != 0 ) {
                $args->walker->active_submenu_item = $item;
            }
        }
        return $classes;
    }
    //add_filter( 'nav_menu_css_class', 'foundationpress_active_nav_class', 10, 3 );
endif;
